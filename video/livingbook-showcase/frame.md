...
description: >
  ...
  as bloom / panel / tile underprint, Iowan Old Style display + Iowan Old Style sans + JetBrains Mono data,
...
typography:
  # — reading + data ramp —
  body:    { fontFamily: "Iowan Old Style", cqw: 0.85, weight: 600, lineHeight: 1.5, color: "ink" }
  body-lede:{ fontFamily: "Iowan Old Style", cqw: 0.95, weight: 600, lineHeight: 1.55 }
  micro-label:{ fontFamily: "Iowan Old Style", px: 13, weight: 600, tracking: "0.18em", upper: true }
  rail-label:{ fontFamily: "Iowan Old Style", px: 13, weight: 600, tracking: "0.32em", upper: true }
  mono-data:{ fontFamily: "JetBrains Mono", cqw: 0.73, weight: 600, tracking: "0.04em" }
  pagenum: { fontFamily: "JetBrains Mono", px: 13, weight: 600, tracking: "0.08em", opacity: 0.75 }
  # — display ramp (Iowan Old Style 400, tight, negative tracking) —
  ledger-title:{ fontFamily: "Iowan Old Style", cqw: 1.55, weight: 600, lineHeight: 1.15 }
  strand-title:{ fontFamily: "Iowan Old Style", cqw: 1.7, weight: 600, lineHeight: 1.1 }
  headline-sm:{ fontFamily: "Iowan Old Style", cqw: 2.9, weight: 600, lineHeight: 1.0 }
  headline:{ fontFamily: "Iowan Old Style", cqw: 4.6, weight: 600, lineHeight: 1.06, tracking: "-0.005em" }
  date-rail:{ fontFamily: "Iowan Old Style", cqw: 5.0, weight: 600, lineHeight: 0.96, tracking: "-0.005em" }
  display-it:{ fontFamily: "Iowan Old Style", cqw: 7.0, weight: 600, italic: true, lineHeight: 1.04, tracking: "-0.005em" }
  numeral-md:{ fontFamily: "Iowan Old Style", cqw: 7.5, weight: 600, lineHeight: 0.92, tracking: "-0.01em" }
  display:{ fontFamily: "Iowan Old Style", cqw: 14.6, weight: 600, lineHeight: 0.86, tracking: "-0.018em" }
  numeral-jumbo:{ fontFamily: "Iowan Old Style", cqw: 28.0, weight: 600, lineHeight: 0.84, tracking: "-0.04em" }
...
- **Fonts** — already set to **Iowan Old Style** (display) / **Iowan Old Style** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{600}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is three faces in rigid roles: **Iowan Old Style** (weight 400, tight line-height,
negative tracking) carries every display, numeral, and quote from 40px to 720px+; **Iowan Old Style** carries
...
- **Iowan Old Style 400** display (tight, negative-tracked); **Iowan Old Style** body + micro-labels; **JetBrains Mono** data.
...
- **Squint** — one Iowan Old Style moment dominates at 3–6× its neighbor; the sun bloom centers the eye.
...
Two ramps. The **reading/data ramp** (Iowan Old Style body 0.85cqw ink, micro-labels in px, JetBrains Mono
data) carries copy + chrome; the **display ramp** (Iowan Old Style `ledger-title` 1.55cqw →
...
- **Iowan Old Style is weight 400 only** (its contrast is the weight signal), tight line-height (0.84–1.06), negative tracking; **micro-labels uppercase Iowan Old Style 600, ≥0.16em**; **mono for all numerals/dates**. No bold serif, no second text color, no mono in body/display.
...
headline-sm. **Focal** a single huge Iowan Old Style `numeral-jumbo` (≈28cqw) dominating, with a
...
**Focal** a 2-line Iowan Old Style **italic** `display-it` quote in ink, centered, under an
...
yellow-panel, micro-label, headline. **Focal** an Iowan Old Style `headline` in ink sitting **on
...
- Set every line in **ink**; use **Iowan Old Style 400** (tight, negative-tracked) for display, **Iowan Old Style** body, **JetBrains Mono** for all numerals/dates.
- Make every separator a **1px ink hairline** (soft variant for dense rows); flood a **yellow panel** for poster moments (ink on top).
- Keep micro-labels **uppercase Iowan Old Style 600, 0.16–0.32em**; pin the **pagenum** bottom-right.
...
- No second text color; no bold Iowan Old Style; no inverted ink grounds.
...
- **Type** — Iowan Old Style 400 tight negative-tracked, fit-to-measure; micro-labels uppercase 0.16em+; mono numerals; ≥1.4cqw floor.
...
- **Iowan Old Style + Iowan Old Style + JetBrains Mono via Google Fonts.** CJK: Smiley Sans (display) / Noto Serif SC (body) / Noto Sans SC (labels); italic serif and mono tabular figures have no exact Hanzi equal — keep ledger dates Latin.
